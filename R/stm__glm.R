...
stm__glm = function(p=NULL, dat=NULL, pa=NULL, variablelist=FALSE, ... ) {
  ...
  if ( exists("stm_local_model_distanceweighted", p) ) {
    if (p$stm_local_model_distanceweighted) {
      hmod = try( glm( p$stm_local_modelformula, data=dat, weights=Y_wgt  ) ) 
    } else {
      hmod = try( glm( p$stm_local_modelformula, data=dat ) ) 
    }
  } else {
      hmod = try( glm( p$stm_local_modelformula, data=dat ) ) 
  } 
  ...
  stm_stats = list( sdTotal=sdTotal, rsquared=rsq, ndata=nrow(dat) ) # must be same order as p$statsvars .. pseudo rsquared for logistic .. for poisson {1- logLik(mod) / logLik(mod_saturated)} might be better
