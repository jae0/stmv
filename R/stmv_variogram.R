...
stmv_variogram = function( xy=NULL, z=NULL, plotdata=FALSE, methods=c("fast"), distance_cutoff=NA, nbreaks = 15, family="gaussian", stanmodel=NULL ) {
  ...
  # -------------------------
      ...
      if ( 0 ) {
        ...
        gr = stmv_variogram( xy, z, methods="fast.rf", plotdata=TRUE ) # nls
  ...
  if ( "fast" %in% methods)  {
    ...
    # nu=0.5, discretized_n=100, nbreaks = 13
      ...
      maxdist = out$stmv_internal_scale/4   # begin with this (diagonal)

      # gives a fast stable empirical variogram using nl least squares

      XY = as.data.frame(g$xy / out$stmv_internal_scale )  # drange keeps things smaller in value to avoid floating point issues
...
---
  ...
  if ( "fast.rf" %in% methods)  {
    ...
    if( 0) {
      ...
      scale = matern_phi2phi( mRange=out$fast.rf$phi, mSmooth=out$fast.rf$nu, parameterization_input="stmv", parameterization_output="RandomFields" )
