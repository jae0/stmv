    ...
    # nu=0.5, discretized_n=100, nbreaks = 13
      ...
      if ( fit$summary$range_ok ) {
        ...
        while ( cnt < 5  ) {
          ...
          if ( out$fast$range < out$vgm$dist0*0.99) out$fast$range_ok = TRUE
      ...
      if (plotdata) {
        ...
        plot.new()
        plot( gamma ~ dist0, data=out$fast$vgm, xlim=c(0,xub),
             ylim=c(0,max(out$fast$vgm$gamma)*1.1), col="blue", pch=20 )
