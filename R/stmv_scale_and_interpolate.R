...
stmv_scale_and_interpolate = function( ip=NULL, p, debugging=FALSE, eps=1e-6, ... ) {
  #\\ core function to interpolate (model variogram) in parallel

### TODO:: placemarker ... scale and interplaoted in one go with fft ... pure space only
## this is just a copy of stmv_scale to get things started ..
...
# main loop over each output location in S (stats output locations)
  for ( iip in ip ) {
    ...
    if ( iip %in% logpoints )  slog = stmv_logfile(p=p, flag="Scale and interpolate determination" )
    ...
    for ( ss in 1:length(p$stmv_variogram_nbreaks_totry) ) {  # different number of breaks actually has an influence upon the stability of variograms
      o = try( stmv_variogram(
        ...
        discretized_n = trunc(stmv_distance_cur / p$pres),
