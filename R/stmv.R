...
stmv = function( p, runmode=NULL, DATA=NULL, nlogs=100, niter=1,
  ...
  for ( nn in 1:niterations ) {
    ...
    if ("interpolate_distance_basis" %in% runmode ) {
      ...
      p0 = p
      dists = p$stmv_distance_scale
      dists = dists[ which( dists >= min( p$stmv_distance_prediction_range ) & dists <= max( p$stmv_distance_prediction_range ) )]
      if (length(dists) > 0 ) {
        for ( j in 1:length(dists) ) {
          p = p0 #reset
          p$stmv_interpolation_basis_distance = dists[j]
