 ...
 # removed incidence matrix method and covariate predictors
  ...
  for (i in 2:Nmc) {
    ...
    #          -log(1),  # Pmuy
          ...
          wFT[] = .C("TSreal_fft_ecmei", n=as.integer(n), T= as.integer(nt), yh=as.double(spate::TSmat.to.vect(y)), inverse=1L, 
         ...
         if (i==2){
          ...
          ffbs = .C("ffbs_spectral_ecmei", wFT=as.double(wFT), bw=as.double(FALSE), ll=as.double(TRUE), 
            ...
            T=as.integer(nt), NFc=as.integer(NFc), ns=as.integer(Z$ns), PACKAGE="ecmei" )

          ffbs_j = .C("ffbs_spectral_ecmei", wFT=as.double(wFT), bw=as.double(FALSE), ll=as.double(TRUE), 
      ...
      if (runif(1)  <= al ) {
        ...
        yhat = spate::vect.to.TSmat( .C("TSreal_fft_ecmei", n=as.integer(n), T=as.integer(nt), yh=as.double(ffbs_j$wFT), inverse=0L, 
                indCos=as.integer(Z$indFFT$indCos), indW=as.integer(Z$indFFT$indW), indWCon=as.integer(Z$indFFT$indWCon),
                NFc=as.integer(NFc), PACKAGE="ecmei" )$yh, T=nt)
