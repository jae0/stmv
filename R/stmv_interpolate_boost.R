...
stmv_interpolate_boost = function( ip=NULL, p,  debugging=FALSE, ... ) {
  #\\ based upon core function to interpolate (model and predict) in parallel .. but force all by boosting with predictions
  ...
  for ( iip in ip ) {
    ...
    if (nrow(pa_fc) > 1) {
      ...
      if ( ngood > 1) {
        ...
        if ( (ngood + ndata ) > p$stmv_interpolate_nmax ) {
          nmore = p$stmv_interpolate_nmax - ndata
