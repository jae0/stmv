...
stm__habitat = function( p, dat, pa ) {
   #\\ this is the core engine of stm .. localised space-time habiat modelling
  ...
  if ( exists("stm_local_model_distanceweighted", p) ) {
    if (p$stm_local_model_distanceweighted) {
      Hmodel = try( gam( p$stm_local_modelformula, data=dat, family=binomial(), weights=weights, optimizer=c("outer","optim")  ) )
    } else {
      Hmodel = try( gam( p$stm_local_modelformula, data=dat, family=binomial(), optimizer=c("outer","optim")  ) )
    }
  } else {
      Hmodel = try( gam( p$stm_local_modelformula, data=dat, family=binomial() ) )
  } 
  ...
  if (rsq < p$stm_rsquared_threshold ) return(NULL)
  ...
  ss = summary(hmod)
  stm_stats = list( sdTotal=sdTotal, rsquared=rsq, ndata=nrow(dat) ) # must be same order as p$statsvars
