...
stm__LaplacesDemon = function( p, dat, pa, sloc, distance, nu, phi, varObs, varSpatial ) {
  ...
  SV = c(rho0 = 0.25, 
    ...
    nburnin = p$stm_spate_nmcmc - p$stm_spate_nposteriors


    if (0) {
      require(LaplacesDemonCpp)
      y=w; yvar=sdTotal^2; n=nsq; Nmc=p$stm_spate_nmcmc;SV=SV; Drift=TRUE; Diffusion=TRUE; NPosteriors=1000; BurnIn=100; NcovUpdates=20; RWCov=NULL; parh=NULL; indEst=1:9; dt=1; MultCov=0.5; ogInd=c(1, 2, 3, 4, 5, 9); nu=1; seed=1
    } 
   ...
   # removed incidence matrix method and covariate predictors
    ...
    if (!is.null(seed)) set.seed(seed)
        ...
        # fast spin up
         ...
         rm(g); gc()
          ...
          source( "~/bio/stm/R/stm_LaplacesDemon_spatemodel.R")
          source( "~/bio/stm/R/stm_LaplacesDemon_Specification.R")
    ...
    return( list( predictions=newdata, stm_stats=stm_stats ) )
