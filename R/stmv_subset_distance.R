...
stmv_subset_distance = function( Si, p, distance_to_upsample ) {
  ...
  o = NULL
  svmethod = p$stmv_variogram_method
  o = try( stmv_variogram( xy=Yloc[yiu,], z=Y[yiu,], methods=svmethod, distance_cutoff=stmv_distance_cur, nbreaks=15, range_correlation=p$stmv_range_correlation ) )
  ...
  if ( !exists("range_ok", o[[svmethod]]) ) {
    out$flag =  E[["variogram_range_limit"]]
    return( out )     #
  }

  if ( !o[[svmethod]][["range_ok"]] ) {
    # retain crude estimate and run with it
    out$flag =  E[["variogram_range_limit"]]
    return(out)
  }

  vario_stmv_distance_cur = o[[svmethod]][["range"]]
