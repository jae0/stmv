...
stm_LaplacesDemon_Specification = function( Algorithm, LogFile, Specs, LIV, Covar, ScaleF, ... ) {
    ...
    if(Algorithm == "ADMG") {
         ...
         mcmcFN = .stm__mcmcadmg
         if( is.null(Specs))
              Specs <- list(n=0, Periodicity=1)
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs), c("n","Periodicity")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         Specs[["n"]] <- abs(round(Specs[["n"]]))
         Specs[["Periodicity"]] <- max(abs(round(Specs[["Periodicity"]])), length(Initial.Values))
         }
         ...
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs), c("Grid","dparm","smax","CPUs","Packages","Dyn.libs")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         ...
         if(!is.null(Specs[["dparm"]])) {
              Specs[["dparm"]] <- unique(interval(round(Specs[["dparm"]]), 1, length(Initial.Values)))
         ...
         mcmcFN = .stm__mcmcahmc

         # Algorithm <- "Adaptive Hamiltonian Monte Carlo"
         if( is.null(Specs))
              Specs=list(epsilon=rep(1/length(Initial.Values), length(Initial.Values)), L=2, Periodicity=1)
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs),c("epsilon","L","Periodicity")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         Specs[["epsilon"]] <- as.vector(abs(Specs[["epsilon"]]))
         if(length(Specs[["epsilon"]]) != length(Initial.Values)) {
              cat("\nLength of epsilon is incorrect.\n", file=LogFile, append=TRUE)
              Specs[["epsilon"]] <- rep(Specs[["epsilon"]][1], length(Initial.Values))}
         ...
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs), c("Nc","Z","beta","CPUs","Packages","Dyn.libs")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         Specs[["Nc"]] <- max(abs(round(Specs[["Nc"]])), 3)
         if(!is.null(Specs[["Z"]])) {
              if(is.matrix(Specs[["Z"]])) {
                   if(ncol(Specs[["Z"]]) != length(Initial.Values))
                        stop("Z has the wrong number of columns.", file=LogFile, append=TRUE)
                   if(nrow(Specs[["Z"]]) != Specs[["Nc"]])
                        stop("Z has the wrong number of rows.", file=LogFile, append=TRUE)}}
         ...
         if(Specs[["CPUs"]] > 1 & Specs[["Nc"]] %% 2 != 0)
              stop("For CPUs > 1, Nc must be even.", file=LogFile, append=TRUE)
         }
        ...
        mcmcFN = .stm__mcmcamm
         ...
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs),                    c("Adaptive","B","n","Periodicity","w")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         if(!is.null(Specs[["B"]])) {
              if(is.null(Covar)) {
                   Covar <- list(NULL)
         ...
         if(!is.null(Specs[["Z"]])) {
              if(is.matrix(Specs[["Z"]])) {
                   ...
                   if(nrow(Specs[["Z"]]) != (floor(Iterations/Thinning)+1)) {
                        ...
                        if(nrow(Specs[["Z"]]) < (floor(Iterations/Thinning)+1)) {
                             Specs[["Z"]] <- rbind(Specs[["Z"]], Specs[["Z"]][1:(floor(Iterations/Thinning)+1-nrow(Specs[["Z"]])),])
                             }
                        else if(nrow(Specs[["Z"]]) > (floor(Iterations/Thinning)+1))
                             Specs[["Z"]] <- Specs[["Z"]][1:(floor(Iterations/Thinning)+1),]
                        Specs[["Z"]][1,] <- Z.temp
                        }
                   Specs[["Z"]] <- array(Specs[["Z"]], dim=c(floor(Iterations/Thinning)+1,                              length(Initial.Values), Specs[["Nc"]]))}
              if(dim(Specs[["Z"]])[1] != floor(Iterations/Thinning)+1)
                   stop("The first dimension of Z is incorrect.", file=LogFile, append=TRUE)
              if(dim(Specs[["Z"]])[2] != length(Initial.Values))
                   stop("The second dimension of Z is incorrect.", file=LogFile, append=TRUE)
              if(dim(Specs[["Z"]])[3] != Specs[["Nc"]])
                   stop("The third dimension of Z is incorrect.", file=LogFile, append=TRUE)}
         ...
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs),                    c("Grid","dparm","CPUs","Packages","Dyn.libs")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         if(is.list(Specs[["Grid"]])) {
              if(length(Specs[["Grid"]]) != length(Initial.Values)) {
                   Specs[["Grid"]] <- list(NULL)
         ...
         alg = "Gibbs Sampler"
         mcmcFN = .stm__mcmcgibbs
         LogMarginalLikelihood = TRUE
         # Algorithm <- "Gibbs Sampler"
         if(is.null(Specs) ) {
              cat("\nSpecs does not exists, Algorithm changed to MWG.\n", file=LogFile, append=TRUE)                         
              Algorithm == "MWG"
              Specs <- NULL
              }
         else {
              ...
              if(!is.function(Specs[["FC"]]))
                   stop("FC must be a function.", file=LogFile, append=TRUE)
              FCtest <- try(Specs[["FC"]](Initial.Values, Data),                         silent=TRUE)
              if(inherits(FCtest, "try-error"))
                   stop("Error in FC.", file=LogFile, append=TRUE)
              if(!is.vector(FCtest))
                   stop("FC must return a vector.", file=LogFile, append=TRUE)
              if(length(FCtest) != length(Initial.Values))
                   stop("Length of parameters to/from FC differs.", file=LogFile, append=TRUE)
              if(!is.null(Specs[["MWG"]]) &
                   !is.vector(Specs[["MWG"]]) &
                   !is.numeric(Specs[["MWG"]]))
                   stop("MWG must be a numeric vector.", file=LogFile, append=TRUE)}               
         }
    else if(Algorithm == "HARM") {
         alg = "Hit-And-Run Metropolis"
         mcmcFN = .stm__mcmcharm
         LogMarginalLikelihood = TRUE
         ...
         if( is.null(Specs))
              Specs=list(epsilon=rep(1/length(Initial.Values), length(Initial.Values)), L=2)
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         ...
         mcmcFN = .stm__mcmchmcda
         # Algorithm <- "Hamiltonian Monte Carlo with Dual-Averaging"
         if(is.null(Specs))
              stop("The Specs argument is required.", file=LogFile, append=TRUE)
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs), c("A","delta","epsilon","Lmax","lambda")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         Specs[["A"]] <- min(round(abs(Specs[["A"]])), Iterations)
         Specs[["delta"]] <- max(min(abs(Specs[["delta"]]), 1),                    1/Iterations)
         ...
         if(length(Specs[["mu"]]) != length(Initial.Values))
              stop("length(mu) != length(Initial.Values).", file=LogFile, append=TRUE)
         }
    else if(Algorithm == "INCA") {
         alg = "Interchain Adaptation"
         mcmcFN = .stm__mcmcinca
         if( is.null(Specs))
              Specs=list(Adaptive=floor(Iterations/2), Periodicity=1)
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         ...
         mcmcFN = .stm__mcmcmala
         LogMarginalLikelihood = TRUE
         # Algorithm <- "Metropolis-Adjusted Langevin Algorithm"
         if( is.null(Specs))
              Specs=list(A=1e7, alpha.star=0.574, gamma=1, delta=1, epsilon=c(1e-6,1e-7))
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs), c("A","alpha.star","gamma","delta","epsilon")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         Specs[["A"]] <- abs(Specs[["A"]][1])
         Specs[["gamma"]] <- min(max(Specs[["gamma"]][1], 0), Iterations)
         Specs[["delta"]] <- min(max(Specs[["delta"]][1], 1e-10), 1000)
         Specs[["epsilon"]] <- abs(Specs[["epsilon"]][1:2])
         }
    else if(Algorithm == "MCMCMC") {
         alg = "Metropolis-Coupled Markov Chain Monte Carlo"
         mcmcFN = .stm__mcmcmcmcmc
         LogMarginalLikelihood = TRUE
         # Algorithm <- "Metropolis-Coupled Markov Chain Monte Carlo"
         if( is.null(Specs))
              Specs <- list(lambda=1, CPUs=1, Packages=NULL, Dyn.libs=NULL)
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs), c("lambda","CPUs","Packages","Dyn.libs")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         Specs[["lambda"]] <- abs(Specs[["lambda"]])
         if(Specs[["CPUs"]] <= 1)
         ...
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs), c("K","CPUs","Packages","Dyn.libs")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         ...
         mcmcFN = .stm__mcmcnuts
         LogMarginalLikelihood = TRUE
         # Algorithm <- "No-U-Turn Sampler"
         if( is.null(Specs))
              Specs=list(A=1e7, alpha.star=0.574, delta=1, epsilon=c(1e-6,1e-7))
         if(is.null(Specs))
              stop("The Specs argument is required.", file=LogFile, append=TRUE)
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs), c("A","delta","epsilon","Lmax")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         Specs[["A"]] <- max(min(round(abs(Specs[["A"]])), Iterations),1)
         Specs[["delta"]] <- max(min(abs(Specs[["delta"]]), 1), 1/Iterations)
         if(!is.null(Specs[["epsilon"]]))
              Specs[["epsilon"]] <- abs(Specs[["epsilon"]][1])
         ...
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs), c("alpha.star","Dist","gamma")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         ...
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs),                    c("bin.n","bin.p","parm.p","selectable","selected")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         ...
         if(Specs[["bin.p"]] < 0 | Specs[["bin.p"]] > 1) {
              Specs[["bin.p"]] <- interval(Specs[["bin.p"]], 0, 1, reflect=FALSE)
         ...
         if(length(Specs[["parm.p"]]) != length(Initial.Values)) {
              Specs[["parm.p"]] <- rep(Specs[["parm.p"]][1], length(Initial.Values))
         ...
         if(length(Specs[["w"]]) != length(Initial.Values))
              Specs[["w"]] <- rep(Specs[["w"]], len=length(Initial.Values))
         if(any(Specs[["w"]] <= 0)) {
         ...
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs), c("epsilon","file","Nr","Nc","size")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         ...
         Specs[["w"]] <- abs(Specs[["w"]])
         if(length(Specs[["w"]]) == 1)
              Specs[["w"]] <- rep(Specs[["w"]], length(Initial.Values))
         ...
         if(is.null(Specs[["SIV"]])) {
              ...
              if(!is.null(Data[["PGF"]])) {
                ...
                Specs[["SIV"]] <- GIV(Model, Data)
              }
         if(!identical(length(Specs[["SIV"]]), length(Initial.Values))) {
         ...
         if(!is.finite(Mo2[["LP"]]))
              stop("SIV results in a non-finite posterior.", file=LogFile, append=TRUE)
         if(!is.finite(Mo2[["Dev"]]))
              stop("SIV results in a non-finite deviance.", file=LogFile, append=TRUE)
         Specs[["SIV"]] <- Mo2[["parm"]]
         rm(Mo2)
         if(Specs[["n1"]] < 1) {
         ...
         mcmcFN = .stm__mcmcusamwg
         # Algorithm <- "Updating Sequential Adaptive Metropolis-within-Gibbs"
         if(is.null(Specs))
              stop("The Specs argument is required.", file=LogFile, append=TRUE)
         if(!is.list(Specs))
              stop("The Specs argument is not a list.", file=LogFile, append=TRUE)
         if(!identical(names(Specs),                    c("Dyn","Periodicity","Fit","Begin")))
              stop("The Specs argument is incorrect.", file=LogFile, append=TRUE)
         if(!is.matrix(Specs[["Dyn"]]))
              Specs[["Dyn"]] <- as.matrix(Specs[["Dyn"]])
         }
  ...
  if(!is.null(Specs[["Periodicity"]])) {
    ...
    if({Specs[["Periodicity"]] < 1} |
     ...
     if(is.list(Covar) & alg != "Adaptive-Mixture Metropolis" & alg != "Random-Walk Metropolis") {
           Covar <- NULL
           tuning <- NULL
           VarCov <- NULL
      ...
      if(is.matrix(VarCov) & !is.list(VarCov)) {
           ...
           for (b in 1:length(LDspecs$Specs[["B"]])) {
                DiagCovar[LDspecs$Specs[["B"]][[b]]] <- diag(VarCov[[b]])
           }
      }

    if (Algorithm == "AMM" & is.list(VarCov) ) mcmcFN = .stm__mcmcamm.b
...
# ---------------------------------------------------------------------------
# the following are direct copies of LaplacesDemon functions, but renamed 
# to permit more flexible argument passing and the following addition to permits simultaeous prediction

# Data$y[Data$indNA] = Data$yhat[Data$indNA] + RcppZiggurat::zrnorm(Data$nNA) * sqrt(Mo0$parm["tau2"]) # 4-5 X faster than rnorm
# ---------------------------------------------------------------------------



     
.stm__mcmcadmg <- function()
     {
     n <- Specs[["n"]]
     ...
     if(CPUs == 1) {
            ...
            Data$y[Data$indNA] = Data$yhat[Data$indNA] + RcppZiggurat::zrnorm(Data$nNA) * sqrt(Mo0$parm["tau2"]) # 4-5 X faster than rnorm
               ...
               if(iter %% Status == 0)
                    cat("Iteration: ", iter,
                         ...
                         file=LogFile, append=TRUE)
               for (j in sample.int(LIV)) {
                    if(j %in% dparm) Mo0 <- .stm__mcmcggdp(Model, Data, j, Mo0, Grid)
                    else {
          ...
          for (iter in 1:Iterations) {
              ...
              Data$y[Data$indNA] = Data$yhat[Data$indNA] + RcppZiggurat::zrnorm(Data$nNA) * sqrt(Mo0$parm["tau2"]) # 4-5 X faster than rnorm
               ...
               for (j in sample.int(LIV)) {
                    if(j %in% dparm)
                         Mo0 <- .stm__mcmcggdpp(Model, Data, j, Mo0, Grid, cl)
                    else {
     ...
     if(CPUs == 1) {
          ...
          for (iter in 1:Iterations) {
              ...
              Data$y[Data$indNA] = Data$yhat[Data$indNA] + RcppZiggurat::zrnorm(Data$nNA) * sqrt(Mo0$parm["tau2"]) # 4-5 X faster than rnorm
               ...
               for (j in sample.int(LIV)) {
                    if(j %in% dparm) Mo0 <- .stm__mcmcggdp(Model, Data, j, Mo0, Grid)
                    else Mo0 <- .stm__mcmcggcp(Model, Data, j, Mo0, Grid)
          ...
          for (iter in 1:Iterations) {
              ...
              Data$y[Data$indNA] = Data$yhat[Data$indNA] + RcppZiggurat::zrnorm(Data$nNA) * sqrt(Mo0$parm["tau2"]) # 4-5 X faster than rnorm
               ...
               for (j in sample.int(LIV)) {
                    ...
                    else Mo0 <- .stm__mcmcggcpp(Model, Data, j, Mo0, Grid, cl)
                    }
               if(iter %% Thinning == 0) {
                    t.iter <- floor(iter/Thinning) + 1
                    thinned[t.iter, ] <- Mo0[["parm"]]
...
.stm__mcmcggcp <- function(Model, Data, j, Mo0, Grid)
     {
     G <- length(Grid[[j]])
     LP.grid <- rep(0, G)
     prop <- Mo0[["parm"]]
...
.stm__mcmcggcpp <- function(Model, Data, j, Mo0, Grid, cl)
     {
     G <- length(Grid[[j]])
     LP.grid <- rep(0, G)
     LIV <- length(Mo0[["parm"]])
...
.stm__mcmcggdp <- function(Model, Data, j, Mo0, Grid)
     {
     G <- length(Grid[[j]])
     LP.grid <- rep(0, G)
     prop <- Mo0[["parm"]]
...
.stm__mcmcggdpp <- function(Model, Data, j, Mo0, Grid, cl)
     {
     G <- length(Grid[[j]])
     LP.grid <- rep(0, G)
     LIV <- length(Mo0[["parm"]])
