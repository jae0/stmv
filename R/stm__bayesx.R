...
stm__bayesx = function( p=NULL, dat=NULL, pa=NULL, variablelist=FALSE ) {
  ...
  if ( !exists( "stm_local_model_bayesxmethod", p) ) p$stm_local_model_bayesxmethod="MCMC"  # slightly more smoothing than the REML method
 
  hmod = try( bayesx( p$stm_local_modelformula, data=dat, method=p$stm_local_model_bayesxmethod, 
                     family="gaussian" ) )
  ...
  if (ss$r.squared < p$stm_rsquared_threshold ) return(NULL)
  ...
  stm_stats = list( sdTotal=sdTotal, rsquared=ss$r.squared, ndata=nrow(dat),
