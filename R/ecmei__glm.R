...
ecmei__glm = function( p, dat, pa ) {
  ...
  if ( exists("ecmei_local_model_distanceweighted", p) ) {
    if (p$ecmei_local_model_distanceweighted) {
      hmod = try( glm( p$ecmei_local_modelformula, data=dat, weights=Y_wgt  ) ) 
    } else {
      hmod = try( glm( p$ecmei_local_modelformula, data=dat ) ) 
    }
  } else {
      hmod = try( glm( p$ecmei_local_modelformula, data=dat ) ) 
  } 
  ...
  ecmei_stats = list( sdTotal=sdTotal, rsquared=rsq, ndata=nrow(dat) ) # must be same order as p$statsvars .. pseudo rsquared for logistic .. for poisson {1- logLik(mod) / logLik(mod_saturated)} might be better
