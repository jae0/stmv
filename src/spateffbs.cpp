...
// .. moved here to make ecmei operate more flexibly 
...
extern "C" void real_fft_ecmei(int *n, double yh[], int *inverse, int indCos[], int indW[], int indWCon[], int *NFc){
  fftw_complex *in, *out;
  fftw_plan p;
  int i;
...
extern "C" void propagate_spectral_ecmei(double xtp1[], double xt[], double G11C[], double G11[], double G12[], int *NFc, int *ns){
  int j, k;
  for(j=0; j<*ns;j++){
    xtp1[j]=G11C[j] * xt[j];
  }
...
extern "C" void kf_spectral_ecmei(double wFT[], double mtt1[], double mtt[], double rtt1[], double rtt[], double specCosOnly[], double G11C[], double specCosSine[], double G11[], double G12[], double *tau2, int *T, int *NFc, int *ns){
  int t,j,k,l,NF;
  NF=(2 * *NFc + *ns);
  ...
  for(t=0; t<*T;t++){
    l = t * NF; 
    propagate_spectral_ecmei(&mtt1[l], &mtt[l], G11C, G11, G12, NFc, ns);
    for(j=0; j<NF;j++){
      k = l +j ;
...
extern "C" void bs_spectral_ecmei(double simAlpha[], double mtt[], double mtt1[], double rtt[], double rtt1[], double spec[], double G11C[], double G11[], double G12[], int *T, int *NFc, int *ns){
  int NF=(2 * *NFc + *ns);
  int j,k,l,t;
  ...
  for(t=(*T-1); t>0;t--){
    propagate_spectral_ecmei(Prop, AlMinusMtt1, G11C, G11, G12t, NFc, ns);
    for(j=0; j<NF;j++){
      k = t * NF +j ;
      l = (t-1) * NF +j ;
...
extern "C" void ll_spectral_ecmei(double *ll, double wFT[], double mtt1[], double rtt1[],  int *T, int *NF, double *tau2){
  *ll=0;
  int t,j,k,l;
  for(t=0; t<*T;t++){
    l = t * *NF;
...
// [[Rcpp::export]]
extern "C" void TSreal_fft_ecmei(int *n, int *T, double yh[], int *inverse, int indCos[], int indW[], int indWCon[], int *NFc){
  int t;
  for(t=0; t<*T;t++){
    real_fft_ecmei(n, &yh[t * *n * *n], inverse, indCos, indW, indWCon, NFc);
  }
}


extern "C" void ffbs_spectral_ecmei(double wFT[], double *bw, double *ll, double specCosOnly[], double G11C[], double specCosSine[], double G11[], double G12[], double specAll[], double *tau2, int *T, int *NFc, int *ns){
/* , double simAlpha[] */
  int NF=(2 * *NFc + *ns);
  int j;
  ...
  kf_spectral_ecmei(wFT, mtt1, mtt, rtt1, rtt, specCosOnly, G11C, specCosSine, G11, G12, tau2, T, NFc, ns);

  if(*ll==1){
    ll_spectral_ecmei(ll, wFT, mtt1, rtt1, T, &NF, tau2);
   // printf("%f",*ll); 
  }
  if(*bw==1){
    bs_spectral_ecmei(wFT, mtt, mtt1, rtt, rtt1, specAll, G11C, G11, G12, T, NFc, ns);
  }
...
extern "C" void ffbs_spectral_ecmei_oneshot(  
    double yh[], 
    double *ll, 
    double *tau2,
    int indCosOnly[],
  ...
  kf_spectral_ecmei(wFT, mtt1, mtt, rtt1, rtt, specCosOnly, G11C, specCosSine, G11, G12, tau2, T, NFc, ns);
  ll_spectral_ecmei(ll, wFT, mtt1, rtt1, T, NF, tau2);
  bs_spectral_ecmei(wFT, mtt, mtt1, rtt, rtt1, spec, G11C, G11, G12, T, NFc, ns);
...
Testing:
  ...
  install_git( "/home/jae/bio/ecmei", branch="clibs", force=TRUE) 
...
microbenchmark::microbenchmark( 
  ...
  {  ffbs_proposal = .C( "ffbs_spectral_ecmei_oneshot", 
...
{
    u = .C("TSreal_fft_ecmei", n=as.integer(n), T= as.integer(T), yh=as.double(spate::TSmat.to.vect(y)), inverse=as.integer(1), 
                  indCos=as.integer(Z$indFFT$indCos), indW=as.integer(Z$indFFT$indW), indWCon=as.integer(Z$indFFT$indWCon), 
                  NFc=as.integer(NFc), PACKAGE="ecmei")$yh
        ...
        mll <- .C("ffbs_spectral_ecmei", wFT=as.double(u), bw=as.double(FALSE), ll=as.double(TRUE), 
                  ...
                  T=as.integer(T), NFc=as.integer(NFc), ns=as.integer(Z$ns), PACKAGE="ecmei" )$ll
        mll

         v = .C("ffbs_spectral_ecmei", wFT=as.double(u), bw=as.double(TRUE), ll=as.double(FALSE), 
              specCosOnly=as.double(spec[indCosOnly]), G11C=as.double(G11C), specCosSine= as.double(spec[Z$indFFT$indCos]), 
              G11=as.double(G11), G12=as.double(G12), specAll=as.double(spec), tau2=as.double(parh_proposal["tau2"]), 
              T=as.integer(T), NFc=as.integer(NFc), ns=as.integer(Z$ns), PACKAGE="ecmei" )$wFT

          yhat = spate::vect.to.TSmat( .C("TSreal_fft_ecmei", n=as.integer(n), T=as.integer(T), yh=as.double(v), inverse=as.integer(0), 
              indCos=as.integer(Z$indFFT$indCos), indW=as.integer(Z$indFFT$indW), indWCon=as.integer(Z$indFFT$indWCon),
              NFc=as.integer(NFc), PACKAGE="ecmei" )$yh, T=T)
}
, times=1000)
